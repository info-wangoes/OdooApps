...
class RestAuth2Access(http.Controller):
    ...
    def auth_url(self, **kw):
        ...
        if user.login == 'public':
            ...
            client_id = request.env['auth.auth'].search([('consumer_key','=',kw['client_id']), ('user_id', '=', user.id)])
            if client_id:
                redirect_url = request.env['redirect.uri'].search([('redirect_id', '=', client_id.id), ('url','=',kw['redirect_uri'])])
                if redirect_url:
                    ...
                    return self._response({'Location': '{url}?{params}'.format(url=redirect_url.url, params=werkzeug.url_encode(params))}, None, 302)
                else:
    ...
    def access_token(self,context=None, **kw):
        ...
        client_id = request.env['auth.auth'].search([('consumer_key','=',kw['client_id']), ('auth_code', '=', kw['code'])])
        if client_id:
            redirect_url = request.env['redirect.uri'].search([('redirect_id', '=', client_id.id), ('url','=',kw['redirect_uri'])])
            if redirect_url:
...
class RestController(http.Controller):
    ...
    def api_get_employee(self, model='hr.employee', values=None, context=None, token=None, **kw):
        ...
        try:
            ...
            return Response(json.dumps({'error': e.__str__() ,'status_code':500},
                sort_keys=True, indent=4), 
                content_type='application/json;charset=utf-8', status=200) 
